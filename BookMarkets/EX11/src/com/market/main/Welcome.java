...
public class Welcome {
	...
	// 2. 장바구니 담긴 상품 목록 보기
	private static void menuCartItemList() throws CarException  {
		if (mCart.mCartCount >= 0)
			mCart.printCart();
		// 장바구니 처리 로직
        boolean errorOccurred = true; // 예시로 오류 발생 여부 설정
        if (errorOccurred) {
            throw new CarException("장바구니에 문제가 발생했습니다.");
        }
	}
	
	// 3. 장바구니 비우기
	private static void menuCartClear() throws CarException {
	...
	private static void menuCartAddItem(Book[] book) {
		...
		out: while (true) {
			...
			// 사용자가 입력한 isbn 번호의 도서가 도서관(Book[]) 몇 번째의 책인지 확인 -> 존재하는 책이 아니면 -1이 담김
	...
	// 5. 장바구니 항목 수량 줄이기
	private static void menuCartRemoveItemCount()throws CarException {
	...
	private static void menuCartBill() {
		...
		else { // 장바구니에 결제할 항목이 있는 경우
			System.out.println("배송받을 분은 고객 정보와 같습니까? Y | N");
			Scanner input = new Scanner(System.in); // 사용자에게 입력받을 수 있도록 객체 준비
			String str = input.nextLine(); // 입력 받은 값을 str 변수에 할당 == 사용자가 입력한 값
			
			if(str.equalsIgnoreCase("Y")) { // 고객 정보가 동일한 경우
				System.out.println("배송지를 입력해 주세요.");
				String address = input.nextLine(); // 사용자가 입력한 주솟값
				
				printBill(mUser.getName(), String.valueOf(mUser.getPhone()), address);
			}
			else { // 고객 정보가 동일하지 않은 경우
