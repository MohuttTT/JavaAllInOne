...
/*        // where(board.title.contains("1") => title 필드가 1을 포함하는 레코드를 조회하는 건을 추가 (WHERE title LIKE '%1%')
    ...
    // Board 목록(페이지네이션) + 댓글 수 + 첨부파일
    /*
        Tuple Query?
        JPQL, QueryDSL에서 사용되는 개념으로, 다양한 데이터 타입을 한 번에 조회할 수 있게 해 주는 기능
        튜플은 여러 컬럼의 결과를 묶어서 하나의 객체로 반환하는 것을 의미

        QueryDSL Tuple
        -- 조회된 여러 컬럼의 값을 각각의 필드로 보유
    */
    @Override
    public Page<BoardListAllDTO> searchWithAll(String[] types, String keyword, Pageable pageable) {
        ...
        /*기본 JPQL 쿼리 작성*/
        JPQLQuery<Board> boardJPQLQuery = from(board); // -- Board 엔티티 기준 쿼리 작성
        boardJPQLQuery.leftJoin(reply).on(reply.board.eq(board)); // -- Reply 엔티티를 Board 엔티티와 left join
        ...
        boardJPQLQuery.groupBy(board); // -- Board 엔티티 기준으로 그룹화 (Board에 대한 집계 쿼리 수행 위함)

        /*페이지네이션 적용*/
        getQuerydsl().applyPagination(pageable, boardJPQLQuery); // paging

        /*튜플 쿼리 작성*/
        // Board 엔티티와 각 Board에 대한 고유한 Reply 개수 선택
        // Tuple은 다양한 타입의 결과를 담을 수 있는 QueryDSL의 결과 타입
        // reply.countDistinct() -- Reply의 개수를 한 번에 조회하는 부분 -- 조인한 다음, 각 Board에 대해 고유한 Reply 개수
        JPQLQuery<Tuple> tupleJPQLQuery = boardJPQLQuery.select(board, reply.countDistinct());

        /*결과 페치 및 DTO 반환*/
        List<Tuple> tupleList = tupleJPQLQuery.fetch(); // -- 쿼리 실행 결과를 Tuple 리스트로 가져옴

        // stream().map() -- Tuple 리스트를 BoardListAllDTO 리스트로 변환
        List<BoardListAllDTO> dtoList = tupleList.stream().map(tuple -> {
            Board board1 = (Board) tuple.get(board); // Board 추출
            long replyCount = tuple.get(1, Long.class); // 댓글 수 추출
        ...
        long totalCount = boardJPQLQuery.fetchCount(); // -- 전체 개수 조회 (쿼리 전체 결과 개수 조회)

        return new PageImpl<>(dtoList, pageable, totalCount); // -- 페이지네이션된 결과 반환
    }
